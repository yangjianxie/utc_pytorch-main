...
class Trainer:
    ...
    def save_final_model(self):
        if self.fabric.is_global_zero:
            model_path = os.path.join(self.checkpoint_dir, "pytorch_model.bin")
            # 保存整个模型，而非仅仅是状态字典
            self.fabric.save(model_path, self.model.state_dict())
            self.fabric.print(f"Final model saved to {model_path}")
    ...
    def fit(
        ...
        self.save_final_model()  # 在训练结束后保存最终模型
